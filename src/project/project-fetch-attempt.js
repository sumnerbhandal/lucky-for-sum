...
import hpPreload from "./components/preload.js"
...
const Portfolio = lazy(() => import('../home/components/portfolio-illustration'));
const EnrichedSearch = lazy(() => import('../home/components/search-illustration'));
const Ligamend = lazy(() => import('../home/components/ligamend-illustration'));
const Heuristic = lazy(() => import('../home/components/heuristic-illustration'));

const Project = (props) => {

    const [pageFeed, setPageFeed] = useState(null);
    const verticalPosition = props.position == null ? 0 : props.position.y;
    const leftPosition = props.position == null ? 0 : props.position.x;
    // const projectIndex = window.location.href.split('pid-')[1] === undefined || pageFeed === null ? 0 : window.location.href.split('pid-')[1];
    // const jSONReferrer = window.location.href.split('pid-')[1] === undefined || pageFeed === null ? hpPreload : pageFeed;
    ...
    useEffect(() => {
        if (!window.location.href.split('pid-')[1] === undefined) {
            console.log("proceed");
        }
        fetch("/project-feed.json")
    ...
    //         if (nextProjectIndex < pageFeed.length) {
    //             loaderBlob.classList.add("loading");
    //             projectsShown.push(pageFeed[nextProjectIndex]);
    ...
    return (
        <div ref={props.target}>
            ...
            {!pageFeed ? (
                ...
                pageFeed.projectPageFeed.map((pdp, index) => (
                    <div key={index} className="project-preview-container header" >
                        <div className="project-preview">
                            <div style={projectPreviewPosition} className="project-preview-thumbnail header">
                                <Suspense fallback={<div></div>}>
                                    {pdp.id === 0 ? <Portfolio /> : index === 1 ? <Ligamend /> : index === 2 ? <EnrichedSearch /> : index === 3 ? <Heuristic /> : null }
                                </Suspense>
                            </div>
